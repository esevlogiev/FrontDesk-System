...
class RoomManagementTest(unittest.TestCase):
    ...
    def test_release_all_rooms(self):
        ...
        cursor.execute('''Select * from Rooms''')
        rooms_rent = [convert_room(room[0]).is_rent for room in cursor.fetchall()]
    ...
    def test_getting_room_bath_items(self):
        ...
        self.assertNotEqual([bath_item, other_bath_item], other_room_bath_items)
    ...
    def test_change_food(self):
        ...
        self.assertFalse(all(food.is_expire() == False for food in foods))
        self.assertTrue(any(food.is_expire() == True for food in foods))

        change_food(TEST_DATABASE_FILENAME)
        cursor.execute('''Select * from Food''')
        foods = [convert_to_food(food[0]) for food in cursor.fetchall()]
        self.assertTrue(all(food.is_expire() == False for food in foods))
        self.assertFalse(any(food.is_expire() == True for food in foods))
        connection.close()
    ...
    def test_change_bath_item(self):
        ...
        self.assertFalse(all(bath_item.has_to_replace() == False\
                         for bath_item in bath_items))
        self.assertTrue(any(bath_item.has_to_replace() == True\
                        for bath_item in bath_items))
        ...
        bath_items = [convert_bath_item(bath_item[0])\
                      for bath_item in cursor.fetchall()]
        self.assertTrue(all(bath_item.has_to_replace() == False\
                        for bath_item in bath_items))
        self.assertFalse(any(bath_item.has_to_replace() == True\
                         for bath_item in bath_items))
        connection.close()
